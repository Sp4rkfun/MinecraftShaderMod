...
vec3 normal_from_depth(vec2 tex) {
  ...
  if(texture2D(gdepth, tex.xy + offset1).g <1.0){
		valid =1;
	}
  if(texture2D(gdepth, tex.xy + offset2).g <1.0){
		valid=1;
	}
...
/* DRAWBUFFERS:4 */
	...
	float occ = (op+od+ol+or)/4;//0.0;
	if(abs(op)>0.9 && abs(od) >0.9){
		occ = 1;
	}
	if(ol<0.1 || or<0.1){
		occ = 0;
	}
	if(ol>0.1 && or>0.1 && (abs(op)>0.9 || abs(od)<0.0)){
		occ = 1;
	}
	
	if(texture2D(gdepth, texcoord.xy).g <1.0 || valid >0){
		occ=1.0;
	}
	//occ=getDepth(texcoord.xy)/10;
